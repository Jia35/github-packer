  ...
  const dictionaries = {
    en: {
      ...
      options: {
        ...
        infoTitle: "💡 Tips",
        info1: "Permissions (Fine-grained): Repository access **All repositories**, Permissions **Contents** (Read-only).",
        info2: "Privacy: Token is stored locally in `chrome.storage`. No data is sent to 3rd party servers.",
        info3: "GitHub Docs: ",
    ...
    zh_TW: {
      ...
      options: {
        ...
        infoTitle: "💡 設定提示",
        info1: "必要權限 (Fine-grained)：Repository access 選擇 **All repositories**，Permissions 僅需勾選 **Contents** (Read-only) 即可。",
        info2: "隱私說明：此 Token 僅會儲存在您本地瀏覽器的 `chrome.storage` 中，不會傳送至任何第三方伺服器。",
        info3: "GitHub 官方文件：",
    ...
    ja: {
      ...
      options: {
        ...
        infoTitle: "💡 ヒント",
        info1: "必要な権限 (Fine-grained): Repository access は **All repositories**、Permissions は **Contents** (Read-only) を選択してください。",
        info2: "プライバシーについて: トークンはローカルの `chrome.storage` にのみ保存され、外部サーバーには送信されません。",
        info3: "GitHub ドキュメント: ",
    ...
    ko: {
      ...
      options: {
        ...
        infoTitle: "💡 팁",
        info1: "필요한 권한 (Fine-grained): Repository access는 **All repositories**, Permissions는 **Contents** (Read-only)를 선택하십시오.",
        info2: "개인정보 보호: 토큰은 로컬 `chrome.storage`에만 저장되며 외부 서버로 전송되지 않습니다.",
        info3: "GitHub 문서: ",
